...
NSString * const kDaDaPushApplicationJSONType = @"application/json";

NSString * DaDaPushPercentEscapedStringFromString(NSString *string) {
    static NSString * const kDaDaPushCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kDaDaPushCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kDaDaPushCharactersGeneralDelimitersToEncode stringByAppendingString:kDaDaPushCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[DaDaPushQueryParamCollection class]]) {
        return object;
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        return [DaDaPushSanitizer dateToString:object];
    }
    ...
    else if ([object isKindOfClass:[DaDaPushObject class]]) {
...
- (NSString *) parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [DaDaPushSanitizer dateToString:param];
    }
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone* timeZone = [DaDaPushDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kDaDaPushApplicationJSONType;
        }
