...
#import "DaDaPushLogger.h"
#import "DaDaPushApiClient.h"
#import "DaDaPushJSONRequestSerializer.h"
#import "DaDaPushQueryParamCollection.h"
#import "DaDaPushDefaultConfiguration.h"

NSString *const DaDaPushResponseObjectErrorKey = @"DaDaPushResponseObject";

static NSString * const kDaDaPushContentDispositionKey = @"Content-Disposition";

static NSDictionary * DaDaPush__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * DaDaPush__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = DaDaPush__headerFieldsForResponse(response);
    if(!headers[kDaDaPushContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kDaDaPushContentDispositionKey];
...
@interface DaDaPushApiClient ()

@property (nonatomic, strong, readwrite) id<DaDaPushConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation DaDaPushApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static DaDaPushApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[DaDaPushDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[DaDaPushDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<DaDaPushConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<DaDaPushConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[DaDaPushResponseDeserializer alloc] init];
        _sanitizer = [[DaDaPushSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        DaDaPushJSONRequestSerializer * swgjsonRequestSerializer = [DaDaPushJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kDaDaPushApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        DaDaPushDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[DaDaPushResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = DaDaPush__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<DaDaPushSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = DaDaPushPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = DaDaPushPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, DaDaPushPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[DaDaPushQueryParamCollection class]]){
            DaDaPushQueryParamCollection * coll = (DaDaPushQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = DaDaPushPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = DaDaPushPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = DaDaPushPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<DaDaPushConfiguration> config = self.configuration;
